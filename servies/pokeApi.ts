...
export async function _getPokeApi(url: string, params: any): Promise<any> {

  return await $fetch(`/api/poke/${url}`, {
    params,
  });
}

export function getPokeApi(url: string, params: any): Promise<any> {
  ...
  if (!promiseCache.has(hash)) {
    const apiPromise = _getPokeApi(url, params)
      .then((res) => {
        state.value = res;
        return res;
      })
